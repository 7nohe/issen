...
表には全 20 ルールを載せています。内訳は、プリセットから移植した 17 ルール（既定で有効で、`--list-rules` に出るもの）と、文書検証 3 種です。このうち 12 のルールは日本語の形態素や表記を見るため、日本語の文書にだけ働きます。残りは言語を選びません。
...
CI は Linux・macOS・Windows でテストを回します。あわせてフォーマットと clippy を確認し、宣言した `rust-version` でビルドします。最後にこのリポジトリ自身の日本語ドキュメントを issen にかけます。

## バージョニング

issen はセマンティックバージョニングに従います。ただし他人の CI が出力に依存するリンターとして解釈しています。バージョン番号が保護するのは、コマンドライン・`issen.yml`・JSON 出力の 3 つです。0.x の間は Rust のライブラリ API を安定させません。

- マイナーリリースは、それまで通っていたビルドを落とすことがある。既定で有効なルールの追加や、`rust-version` の引き上げが該当する。0.x の間は、コマンドライン・設定・JSON 出力の互換性のない変更もマイナーリリースで行い、変更履歴に明記する
- パッチリリースはバグを直す。issen が誤った報告をしていた場合、その修正で報告内容が変わり、検出が増えることもある
- JSON 出力へのフィールド追加は破壊的変更ではない。削除や改名は破壊的変更である

検出内容の変化をすべて意図したアップグレードとして扱いたい場合は、CI でバージョンを完全に固定してください。変更は [CHANGELOG.md](CHANGELOG.md) に記録します。
